...
| QQ 机器人 | 静态配置 | `QQ_APP_ID`, `QQ_CLIENT_SECRET` | `QQ_USER_OPENID`, `QQ_GROUP_OPENID` | 使用 QQ 开放平台 Bot API；AppID + AppSecret 认证；OpenID 可通过 `scripts/get_qq_openid.py` 获取；至少配置一个发送目标（私聊或群聊） |
...
| `QQ_APP_ID` | minimal | qq | Secret | - |
| `QQ_CLIENT_SECRET` | minimal | qq | Secret | - |
| `QQ_USER_OPENID` | advanced | qq | Secret | - |
| `QQ_GROUP_OPENID` | advanced | qq | Secret | - |
